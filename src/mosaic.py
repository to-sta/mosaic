...
def generate_mosaic(base_image_path: str, mosaic_images_path: str, desired_height: int, desired_width: int, filetyp: str='png') -> None:
    ''' 
    ...
    for _ in range(end_height):
        for _ in range(end_width):
            ...
            if base_img.shape[1] < x + desired_width or base_img.shape[0] < y + desired_height:
    ...
    cv.imwrite(f'img_h{desired_height}w{desired_width}_{uuid.uuid4()}.{filetyp}', base_img)